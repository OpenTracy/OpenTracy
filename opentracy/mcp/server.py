"""
MCP Server implementation for OpenTracy.

This server exposes OpenTracy functionality as MCP tools
for use with Claude Code, Claw, and other MCP-compatible clients.

Usage:
    # Run the server
    python -m opentracy.mcp
...
def create_server():
    ...
    async def handle_generate(args: dict) -> list[TextContent]:
        """Handle opentracy_generate tool."""
    ...
    async def handle_smart_generate(args: dict) -> list[TextContent]:
        """Handle opentracy_smart_generate tool."""
        router = get_router()
        if router is None:
    ...
    async def handle_list_models(args: dict) -> list[TextContent]:
        """Handle opentracy_list_models tool."""
